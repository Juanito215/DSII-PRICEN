...
function Aseo() {
  ...
  const fetchProductos = async (ordenar = null, direccion = null) => {
    try {
      let url = 'http://localhost:3000/api/productos/categoria/aseo';
      ...
      if (ordenar && direccion) {
        ...
        url = 'http://localhost:3000/api/productos/mas-vistos?categoria=aseo';
      }
  ...
  useEffect(() => {
    ...
    const fetchProductoMasEconomico = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/productos/economicos/aseo');
  ...
  const getImage = (name) => `/src/assets/aseo/${name}`;
  ...
  const handleNotesClick = () => {
    ...
    navigate('/notas', { state: { from: '/aseo' } });
  };
        ...
        <section className="featured-product">
          <div className="product-info">
            ...
            <p className="product-description">
              {featuredProduct.descripcion || "Producto de limpieza de alta calidad! Deja el piso brillando y a los visitantes sorprendidos."}
            </p>
